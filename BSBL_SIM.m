function Result = BSBL_SIM(Phi, y, blkStartLoc, LearnLambda, varargin)
...
% Default Parameter Values for Any Cases00
...
if(mod(length(varargin),2)==1)  % varargin length=4
    error('Optional parameters should always go by pairs\n');
else    
    for i=1:2:(length(varargin)-1)   % 1:2:4   i=1£¬3
...
maxLen = max(blkLenList); %20
if sum(blkLenList == maxLen) == p, % equalSize=0
    equalSize = 1;
else
    equalSize = 0;
end
